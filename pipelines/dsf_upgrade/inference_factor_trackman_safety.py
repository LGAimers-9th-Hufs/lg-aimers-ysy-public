#!/usr/bin/env python3
"""DSF + seed-specific FactorTabM router with a frozen TrackMan safety gate."""
...
def main() -> None:
    ...
    dsf_root = root / "dsf"
    spec = importlib.util.spec_from_file_location("_factor_safety_dsf", dsf_root / "member_script.py")
    ...
    factor = _load_predict(
        model_dir / "factor_tabm.pt",
        joblib.load(model_dir / "preprocessor.joblib"),
        base_features,
    )
    ...
    print(f"DSF/FactorTabM safety rows={len(sample)} routed={int(routed.sum())} blocked={int(unsafe.sum())}")
