#!/usr/bin/env python3
"""Build the FactorTabM/TrackMan safety model with residual correction."""
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("./seed1_tm_res.zip"))
    args = parser.parse_args()
    if len(args.output.name) > 20:
        raise ValueError("ZIP filename must be at most 20 characters")
    with tempfile.TemporaryDirectory(prefix="factor_trackman_residual_") as temporary:
        ...
        for source, name in (
            ...
            (args.factor_dir / "residual.joblib", "residual.joblib"),
        ...
        shutil.copy2(HERE / "inference_factor_trackman_residual.py", package / "script.py")
        for relative in (
        ...
        # All other imports are preinstalled in the official evaluation image.
