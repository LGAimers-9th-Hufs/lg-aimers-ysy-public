#!/usr/bin/env python3
"""과거 fold 공통 안정 피처 CatBoost 규정 준수 고정 앙상블."""
...
def load_bundle(model_dir=MODEL_DIR):
    ...
    models=[]
    for name in art["model_files"]:
        model=CatBoostClassifier(); model.load_model(model_dir/name); models.append(model)
    if len(models)!=len(art["weights"]): raise ValueError("모델/가중치 수 불일치")
    return models,art

def predict_frame(df,models,art):
    x=build_features(df)[art["feature_cols"]]
    if x.columns.tolist()!=art["feature_cols"]: raise ValueError("피처 스키마 불일치")
    cat_idx=[x.columns.get_loc(c) for c in art["cat_cols"]]
    pool=Pool(x,cat_features=cat_idx); weights=np.asarray(art["weights"],dtype=np.float64); weights/=weights.sum()
    pred=np.zeros(len(df),dtype=np.float64)
    for model,weight in zip(models,weights):
        pred += weight*model.predict_proba(pool,thread_count=int(art.get("predict_threads",6)))[:,1]
    return calibrate(pred,art["calibration"])

def main():
    models,art=load_bundle(); test=pd.read_csv(DATA_DIR/"test.csv",encoding="utf-8-sig")
